...
# There won't be any special characters ('&', '?', '#' etc.) You need to use 3
# spaces between words and 1 space between each letter of each word.
# 
# Input: The secret message as a string.
# Output: The same string, but encrypted.
# 
...
def morse_encoder(text):
    return ''.join('{} '.format(MORSE.get(c, c)) for c in text.lower()).strip()
